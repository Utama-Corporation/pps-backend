...
// GET /labels/furniture-wip/partial/:noFurnitureWipPartial/pdf
exports.generatePartialPdf = async (req, res) => {
  try {
    const code = String(req.params.noFurnitureWipPartial || "").trim();
    if (!code) {
      return res
        .status(400)
        .json({ success: false, message: "noFurnitureWipPartial wajib diisi" });
    }

    const row = await service.getByNoFurnitureWipPartial(code);
    ...
    console.error("FurnitureWIP Partial PDF Error:", err);
    const status = err.statusCode || 500;
    return res.status(status).json({ success: false, message: err.message });
  }
};
...
exports.incrementPartialHasBeenPrinted = async (req, res) => {
  ...
  try {
    ...
    console.error("Increment FurnitureWIP Partial HasBeenPrinted Error:", err);
    const status = err.statusCode || 500;
    return res.status(status).json({
      success: false,
      message: err.message || "Terjadi kesalahan server",
