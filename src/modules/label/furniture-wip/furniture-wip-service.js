...
// Label partial (BC.xxxxxxxxxx) tidak punya konteks produksi sendiri —
...
exports.getByNoFurnitureWipPartial = async (noFurnitureWipPartial) => {
  const partial = await readRepo.getPartialRow(noFurnitureWipPartial);
  if (!partial) {
    const e = new Error(
      `Label partial ${noFurnitureWipPartial} tidak ditemukan`,
    );
    e.statusCode = 404;
    throw e;
  }

  const parent = await exports.getByNoFurnitureWip(partial.NoFurnitureWIP);

  return {
    NoFurnitureWIP: partial.NoFurnitureWIPPartial,
    NoFurnitureWIPAsal: partial.NoFurnitureWIP,
    DateCreate: parent.DateCreate,
    NamaFurnitureWIP: parent.NamaFurnitureWIP,
    Pcs: partial.Pcs,
    Berat: parent.Berat,
...
exports.incrementPartialHasBeenPrinted = async (payload) => {
  const code = String(payload?.NoFurnitureWIPPartial || "").trim();
  if (!code) throw badReq("NoFurnitureWIPPartial wajib diisi");
  ...
  try {
    ...
    const row = await writeRepo.incrementPartialHasBeenPrinted(tx, code);
    if (!row) {
    ...
    return {
      NoFurnitureWIPPartial: row.NoFurnitureWIPPartial,
      HasBeenPrinted: row.HasBeenPrinted,
    };
  } catch (e) {
    try {
